...
trait F7FieldWithMin extends F7FieldWithMainElem with Mutable {
  ...
  override def updateFieldWithoutReRendering()(implicit form: Form7, fsc: FSContext): scala.util.Try[Js] =
    super.updateFieldWithoutReRendering().map(_ & _min.updateIfChanged({
      case (old, None) => JS.removeAttr(mainElemId, "min")
      case (old, Some(value)) => JS.setAttr(mainElemId)("min", value)
    }, Js.Void))
}
