...
trait F7FieldWithPlaceholder extends F7FieldWithMainElem with Mutable {
  ...
  override def updateFieldWithoutReRendering()(implicit form: Form7, fsc: FSContext): scala.util.Try[Js] =
    super.updateFieldWithoutReRendering().map(_ & _placeholder.updateIfChanged({
      case (old, None) => JS.removeAttr(mainElemId, "placeholder")
      case (old, Some(value)) => JS.setAttr(mainElemId)("placeholder", value)
    }, Js.Void))
}
