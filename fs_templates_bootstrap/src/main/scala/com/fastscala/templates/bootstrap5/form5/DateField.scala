...
import com.fastscala.js.Js
import com.fastscala.templates.bootstrap5.classes.BSHelpers.input_group
...
trait DateFieldOptRenderer {
  ...
  def render(field: DateFieldOpt)(
    ...
    div.withId(field.aroundId) {
      val showErrors = hints.contains(ShowValidationsHint)
      labelOpt.map(_.form_label.withFor(field.elemId)).getOrElse(NodeSeq.Empty) ++
