...
trait F7FieldWithReadOnly extends F7FieldWithMainElem with Mutable {
  ...
  def updateFieldReadOnlyStatus(_readOnly: F7FieldMixinStatus[Boolean])(implicit form: Form7, fsc: FSContext): scala.util.Try[Js] = scala.util.Success(_readOnly.updateIfChanged({
    case (_, true) => JS.setAttr(mainElemId)("readonly", "readonly")
    case (_, false) => JS.removeAttr(mainElemId, "readonly")
  }, Js.Void))
